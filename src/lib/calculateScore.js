export const calculateCreditScore = (user) => {
  // If no info provided → minimum score 15
  if (!user) {
    return {
      totalScore: 15,
      overallPercent: 15,
  ...
  else if (income <= 50000) incomeScore = 10;
  ...
  // ---------- Existing Loan ----------
  const loan = user.existingLoan || 0;
  ...
  // ---------- Ratio Debt / Income ----------
  const ratio =
    income > 0 ? (user.existingLoan || 0) / income : undefined;
  let ratioScore = 0;
  ...
  // ---------- Category ----------
  let category = "Faible"; // Red
  if (overallPercent >= 56 && overallPercent <= 79) category = "Moyen"; // Orange
  else if (overallPercent > 80) category = "Élevé"; // Green

  return {
    totalScore,          // total raw score
    overallPercent,      // 0-100 %
    category,            // Faible | Moyen | Élevé
    breakdown,           // for histogram: score/max
  };
};
